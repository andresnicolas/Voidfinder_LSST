/* Copyright [2021] Andrés Nicolas Ruiz */
...
/* 
Is this a non-const reference? If so, make const or use a
  pointer: T_Healpix_Base<int> &hp 
Is this a non-const reference? If so, make const or use a 
  pointer: vector<voids> &v
*/
void find_centers(float delta_seed, T_Healpix_Base<int> &hp,
                  struct hpmap *map, vector <voids> &v) {
  int i = 0;
  fix_arr < int, 8 > neigh;
  bool center;
...
/*
Is this a non-const reference? If so, make const or use a 
   pointer: vector<tracer> &tr
Is this a non-const reference? If so, make const or use a 
   pointer: vector<tracer> &ran
Is this a non-const reference? If so, make const or use a
   pointer: vector<voids> &v 
*/
void find_voids(float delta_cut, T_Healpix_Base<int> &hp, struct hpmap *map,
                vector <tracer> &tr, vector <tracer> &ran,
                vector <voids> &v) {
...
/*
Using C-style cast.  Use static_cast<float>(...) instead 
 [readability/casting]
*/
...
*/
             ...
             sort_gal = (struct sort *) malloc(dist_gal.size()
                        * sizeof(struct sort));
             sort_ran = (struct sort *) malloc(dist_ran.size()
                        * sizeof(struct sort));
      ...
      if (v[iv].tof) fprintf(stdout, "%f %f %f \n",
          v[iv].radius * G.ShellDist, v[iv].coord_init.phi*RAD2DEG,
          90.0-v[iv].coord_init.theta*RAD2DEG);
  }
}
